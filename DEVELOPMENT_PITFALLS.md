...
- **macOS 后端起不来：GitHub Actions artifact 跨 job 传输丢 sidecar 执行位**：`build-macos` job 里 `chmod +x` 的 sidecar 经 upload-artifact/download-artifact（zip）传输后 Unix 执行位丢失，Tauri bundler 不自动补，打包后 `Contents/MacOS/ai-drama-backend` 是 `-rw-r--r--`，Rust `spawn()` 报 `Permission denied`（退出码 1），后端起不来、但 app 主进程正常打开（非 panic 加固让它不闪退）。修复：release.yml 的 `build-app (macos)` job 在 download-artifact 后、Tauri build 前加 `chmod +x apps/desktop/src-tauri/binaries/ai-drama-backend-*`。验证手段：macOS 冒烟测试（`.github/workflows/smoke-test-macos.yml`）直跑 sidecar 看 `ls -l` 权限位与退出码。
