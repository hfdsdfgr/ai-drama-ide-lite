...
- [x] AI 续写 / 扩写 / 重写（依赖 Phase 3 — AI Provider 基础系统）
- [x] AI 整本撰写向导（题材 / 受众 / 初步想法 / 情节复杂程度 → 大纲 → 逐章生成，每章人工确认）
完成标准
...
- [x] 旧数据懒回填 Asset ID（读取时自动补全）
 Image Spec
- [x] 默认规格：角色 2:3（1024×1536）/ 场景 16:9（1280×720）/ 道具 1:1（1024×1024）
- [x] 生图规格可自定义：图片比例（1:1/2:3/3:4/4:3/16:9/9:16）+ 画风（写实/动漫/国风/水墨/像素/3D/油画等）下拉，保存到资产卡，预览框实时联动
...
- [x] 按镜头汇总脚本、人物 / 场景 / 道具、提示词、关键帧、视频和审核状态。
- [x] 明确显示缺失项、阻塞原因、过期依赖，并可跳到对应编辑位置。
  - 已完成：缺失项 / 失败 / 审核异常与对应资产、分镜跳转；版本依赖按实际使用版本 ID 检查，支持回退、历史固定、来源未知、文件缺失与关键帧到视频的传递判断。
- [x] “准备本集”只补齐缺失结构和执行预检；付费图片 / 视频仍需用户确认。
  - 仅补齐已有剧本字段可确定的空提示词，不覆盖用户 Prompt；接口保证不创建 Job。
- [x] 复用现有 Pipeline / Job / Overview 查询，不另建第二套状态机。

> 实现记录（2026-09-09）：生成中心新增“剧集制作台”，提供按集切换、完成度、逐镜头状态、
> 阻塞项展开与跳转，以及零付费“准备本集”预检。后端以单个 Overview 聚合接口读取现有
> Script / Asset / Version / Job / Review 状态；未新增数据库表或并行状态机。定向后端测试 8 项、
> 后端全量测试 370 项、前端测试 28 项、TypeScript 构建与 oxlint 均通过；
> 1440px / 900px 深色主题布局已做截图检查。

> 依赖健康补充（2026-09-10）：新增 current / stale / pinned / unknown / broken / none
> 六类版本依赖结论。生成请求保存选择意图与精确版本；显式空参考列表不会回退自动匹配；
> 工作台只提示待确认，不自动创建付费 Job。视频沿实际使用的关键帧继续检查其上游资产，
> 资产页和生成中心可直接跳到对应的图片 / 视频配置。后端全量测试 387 项、前端测试 31 项，
> TypeScript 构建、oxlint 与 1440px / 900px 浏览器验收通过。

M2 — 工作流模板与依赖可视化（P1，已完成）

- [x] 保存阶段开关、Capability、模型偏好和生成参数；禁止保存 API Key。
- [x] 模板应用前展示差异，模型不可用时按 Capability 重新推荐，不静默替换。
- [x] 首版提供当前镜头 / 场景的只读依赖图、版本与过期原因；暂不做自由拖拽画布。

> 实现记录（2026-09-11）：新增项目内工作流模板、剧集级配置、逐字段差异选择、修订冲突保护，
> 以及按剧集隔离的分镜 → 关键帧 → 视频 Pipeline。启动时固化模型与参数快照，暂停恢复复用已提交
> 子任务，不因模板或全局默认变化而换模型；小说分析、剧本生成与共享资产仅作前置检查。本轮未加入音频
> 配置，剧集 Pipeline 明确以无声视频提交。只读依赖视图支持镜头 / 场景范围、精确版本、历史固定、
> 过期 / 损坏 / 未记录来源、分页和资产 / 分镜跳转。后端全量测试 396 项、前端测试 31 项，
> TypeScript 构建、oxlint 与 1440px / 900px 浏览器验收通过。详见
> [Phase 24 M2 实施报告](docs/investigations/phase24-m2-implementation-report.md)。
