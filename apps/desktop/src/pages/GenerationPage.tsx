...
import { getEpisodeWorkspace, getProjectOverview, prepareEpisode } from "../api/overview";
import { getProjectQuality, type ProjectQuality } from "../api/quality";
import { getEpisodePipelineStatus, type PipelineStatus } from "../api/pipeline";
...
import { EpisodeWorkflowConfig } from "../components/EpisodeWorkflowConfig";
import { DependencyView, type DependencyScope } from "../components/DependencyView";
  ...
  return (
    <div className="card job-row">
      ...
      <div className="job-actions">
        {job.target_id && onJumpToShot && job.target_type === "shot" && (
          ...
          ["completed", "failed", "cancelled"].includes(job.status) && (
            <button
              ...
              onClick={() =>
                onJumpToShot(
                  job.target_id,
                  job.capability.includes("video") ? "video" : "image",
                )
              }
            >
              更改设置后再生成
...
export function GenerationPage({
  ...
  const [episodeWorkspace, setEpisodeWorkspace] = useState<EpisodeWorkspaceData | null>(
    null,
  );
  const [selectedEpisodeId, setSelectedEpisodeId] = useState("");
  const [preparingEpisode, setPreparingEpisode] = useState(false);
  const [prepareNotice, setPrepareNotice] = useState("");
  const [dependencyScope, setDependencyScope] = useState<DependencyScope | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const episodePipeline = nextJobs.find(
        (job) => job.type === "pipeline" && job.target_type === "episode",
      );
      if (episodePipeline?.target_id) {
        setPipelineJob(episodePipeline);
        try {
          setPipelineStatus(
            await getEpisodePipelineStatus(
              projectId,
              episodePipeline.target_id,
              episodePipeline.job_id,
            ),
          );
        } catch {
          setPipelineStatus(null);
  ...
  async function handleEpisodePipelineStarted(job: JobOut) {
    setPipelineJob(job);
    setPipelineStatus(null);
    await refresh();
  }
  ...
  const selectedEpisode =
    episodeWorkspace?.episodes.find(
      (episode) => episode.episode_id === selectedEpisodeId,
    ) ?? episodeWorkspace?.episodes[0];
      ...
      {!projectId ? (
        ...
        <>
          <EpisodeWorkspace
            ...
            onOpenDependencies={setDependencyScope}
          />
          {dependencyScope && (
            <DependencyView
              projectId={projectId}
              scope={dependencyScope}
              onClose={() => setDependencyScope(null)}
              onJumpToShot={onJumpToShot}
              onOpenAssets={onOpenAssets}
            />
          )}
            ...
            <section className="generation-jobs">
              {selectedEpisode && (
                <EpisodeWorkflowConfig
                  ...
                  onStarted={(job) => void handleEpisodePipelineStarted(job)}
                />
              )}
              {pipelineJob && (
                <section className="card pipeline-panel">
                  <div className="pipeline-run">
                    <p className="muted">
                      本集任务：{STATUS_LABEL[pipelineJob.status]}
