"""SiliconFlow 视频 Adapter：在 OpenAI Compat（chat/image）基础上补齐异步视频。"""
...
class SiliconFlowVideoAdapter(OpenAICompatAdapter):
    name = "siliconflow-video"
    protocol = "siliconflow_video"
    provider_label = "SiliconFlow Video"
    ...
    def submit(
        ...
        body: dict = {"model": ctx.model_id, "prompt": request.prompt}
        if capability == "image_to_video":
            body["image"] = image_to_data_url(request.images[0])

        url = ctx.base_url.rstrip("/") + "/video/submit"
        ...
        if not request_id:
            raise AdapterError(502, "video_submit_invalid", f"{ctx.provider_name} 未返回视频任务 requestId")
        return request_id

    def poll(self, ctx: ProviderContext, job_id: str) -> JobStatus:
        url = ctx.base_url.rstrip("/") + "/video/status"
        headers = {"Authorization": f"Bearer {ctx.api_key}"} if ctx.api_key else {}
        try:
            with httpx.Client(timeout=30) as client:
                response = client.post(url, headers=headers, json={"requestId": job_id})
                response.raise_for_status()
                payload = response.json()
        except httpx.TimeoutException as exc:
        ...
        if status == "failed":
            ...
            videos = results.get("videos") if isinstance(results, dict) else None
            urls = [
                item["url"]
                for item in videos
                if isinstance(item, dict) and item.get("url")
            ] if isinstance(videos, list) else []
            result = GenerationResult(urls=urls, meta={"provider": ctx.provider_name, "status": raw_status})
