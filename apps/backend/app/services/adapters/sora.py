"""OpenAI Sora 视频 Adapter：在 OpenAI Compat（chat/image）基础上补齐异步视频。"""
...
# 前端通用时长 5/10/15 映射到 Sora 支持的 4/8/12 秒。
_SECONDS = {5: "4", 10: "8", 15: "12"}


class SoraVideoAdapter(OpenAICompatAdapter):
    name = "sora"
    protocol = "sora"
    ...
    def submit(
        ...
        body: dict = {
            ...
            "seconds": _SECONDS.get(request.duration or 5, "4"),
        }
        if capability == "image_to_video":
            body["input_reference"] = {
                "type": "input_reference",
                "image_url": image_to_data_url(request.images[0]),
            }
    ...
    def poll(self, ctx: ProviderContext, job_id: str) -> JobStatus:
        ...
        if isinstance(payload.get("progress"), (int, float)):
            progress = float(payload["progress"]) / 100
        ...
        if status == "failed":
            ...
            result_url = payload.get("url") if isinstance(payload, dict) else None
            if result_url:
                result = GenerationResult(urls=[result_url], meta={"provider": ctx.provider_name})
            else:
                content_url = f"{ctx.base_url.rstrip('/')}/videos/{job_id}/content"
        ...
        return JobStatus(job_id=job_id, status=status, progress=progress, error=error, result=result)
