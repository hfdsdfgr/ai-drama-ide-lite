...
- **占位入口（未实现功能）**：用 disabled 按钮 + `title` 提示 + 一行 `muted` 说明，明确标「后续阶段开放」；不做可点击但无反应的假按钮。占位按钮不用 `.btn-primary`，用默认次要样式，待功能上线再替换为可用主操作。
...
## 9. 用户视角易用性检查清单（后续所有 UI 必查）

每做一个界面或交互，先把自己当作用户，按下面顺序检查：

1. **下一步是否可见**：当前主操作必须在当前上下文里明显可找到，不能藏在无关区域。
2. **默认值是否合理**：模型 / Provider / 比例 / 画风等尽量预选用户已配置的默认值；无效选项不要出现。
3. **结果是否就地出现**：保存、生成、导入后的结果应在当前卡片 / 列表 / 预览区直接更新，不要求手动刷新或跳转。
4. **错误是否可理解且就近**：错误显示在触发控件旁，并说明发生了什么、为什么、用户可以怎么办。
5. **空状态是否有下一步**：没有模型 / 项目 / 章节 / 版本时，显示明确的下一步和入口，不用空白页或死按钮。
6. **付费提示是否克制**：可能产生 API 费用时用一句短提示说明，不每次弹确认框。
7. **状态是否真实**：只展示后端真实状态；没有真实进度时显示“处理中……”，禁止假进度条。
8. **文案是否面向用户**：不使用 `INT`、`medium close` 等开发术语，改用“内景”“中近景”等中文行业表达。
9. **切换是否保持状态**：生成参数、选中项、结果缩略图在页面切换 / 重新激活后仍要恢复。
10. **是否遵守已有布局规范**：继续遵守本文件前文的三栏、滚动、按钮层级、响应式规则，不因加功能破坏空间结构。

这条清单适用于 Phase 13 及之后所有阶段，不只是图片生成。
