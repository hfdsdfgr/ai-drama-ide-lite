...
**最新版本：v0.2.7**（Windows x64 + macOS Apple Silicon）
...
在「生成中心」选择剧集后，可以先检查每个镜头的缺失项和版本依赖，再保存本集的分镜、关键帧、视频模型与参数。调通后可保存为当前项目模板；套用到下一集时会先列出差异，由您逐项勾选，不会整套覆盖。开始制作只处理当前剧集，并在启动时固定本次模型和参数；模板操作本身不会创建付费任务。
...
完整计划见 [ROADMAP.md](ROADMAP.md)（Phase 0–24，从项目初始化到剧集生产台）。摘要：
...
**当前进度：MVP 功能已完成并发布 v0.2.7（Windows + macOS 双平台）**，完整计划见 [ROADMAP.md](ROADMAP.md)。
...
- **剧集制作台**：按镜头汇总生产状态，识别缺失、失败和版本依赖待确认项，并定位到图片 / 视频配置
- **Job 系统**：所有耗时任务持久化（排队 / 运行 / 暂停 / 完成 / 失败 / 取消），可中断可恢复
- **API Key 安全**：系统凭据管理器存储，不落项目文件 / 日志 / Git

已发布版本：

- **v0.2.7**：整体 UI 简约化与信息层级优化；支持外部资产 / 参考图导入、生成链路依赖增强和可解释模型推荐
