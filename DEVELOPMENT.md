...
后续所有功能设计都必须先回答四个问题，并把对应的用户路径写进调研 / 计划文档：

1. 用户在这个界面会看到什么？
2. 用户下一步最自然的操作在哪里？
3. 生成或保存的结果会出现在哪里？
4. 出错时用户应该看到什么、下一步做什么？

易用性不是做完功能后再美化，而是功能设计的一部分：

- 主操作放在当前上下文里，默认值合理，模型下拉只显示有效选项。
- 结果就地出现，不要求用户手动刷新或跳转。
- 错误靠近触发位置，给出原因和可执行的下一步。
- 没有可用模型 / 数据时，明确引导用户去配置，而不是留空白或死按钮。
- 涉及付费 API 时用简短提示告知成本，但不做无意义弹窗打断流程。
- 不伪造进度；没有真实百分比时使用“处理中……”这类不确定状态。
- 用户界面不使用开发术语，优先使用中文行业术语。
